...
def run_pdf_parser(pdf_name: str):
    ...
    # Read PDF from input/ (Path is now relative to the PROJECT_ROOT)
    input_path = PROJECT_ROOT / "input" / f"{pdf_name}.pdf"
    ...
    # Save sections (text with tags) (Path is now relative to the PROJECT_ROOT)
    output_sections = PROJECT_ROOT / "output" / f"sections_{pdf_name}.txt"
    ...
    # Save tables/figures (Paths are now relative to the PROJECT_ROOT)
    tables_dir = PROJECT_ROOT / "output" / f"tables_{pdf_name}"
    tables_dir.mkdir(exist_ok=True)
    figures_dir = PROJECT_ROOT / "output" / f"figures_{pdf_name}"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        # NOTE: User should run this command from the mydeepdoc/ folder:
        # python ruparser/run_pdf_parser.py my_document
        sys.exit(1)
